...
func (a *AnomaliesDetector) DetectAnomalies(freqChan chan *message.MessageResponse, stopChan chan struct{}) {

	go func(freqChan chan *message.MessageResponse) {
		var msg *message.MessageResponse
		var ok bool
		...
		for !computed {
			...
			computed = a.InitCalculateMeanSD((*msg).GetFrequency())
		}
		...
		for {
			...
			a.ProcessNextFrequency((*msg).GetFrequency(), (*msg).GetTimestamp())
		}
